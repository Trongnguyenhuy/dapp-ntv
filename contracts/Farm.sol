...
contract TokenFarm {		
	...
	Dappatoken public dappatoken; //token dapp
	Redapptoken public redapptoken;	//token ndt gui
	...
	constructor(Dappatoken _dappatoken, Redapptoken _redapptoken) public {
		dappatoken = _dappatoken;
		redapptoken = _redapptoken;
		owner = msg.sender;
	}
	...
	function stakeTokens(uint _amount) public {				
		...
		redapptoken.transferFrom(msg.sender, address(this), _amount);
	...
	function unstakeTokens() public {
		...
		// transfer Mock Dai tokens to this contract for staking
		redapptoken.transfer(msg.sender, balance);
	...
	function issueTokens() public {
		...
		for (uint i=0; i<stakers.length; i++) {
			...
			if(balance > 0) {
				dappatoken.transfer(recipient, balance);
			}			
		}
	}
